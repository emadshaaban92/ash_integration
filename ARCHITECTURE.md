...
| **Deliver** | Broadway relay claims `:scheduled` rows (soft lease + attempt bump, capped at `max_in_flight_per_connection` live-lease rows per connection so one slow-but-not-failing connection can't monopolize the batch processors), sends them, and reports the outcome. The relay is "dumb" — it only executes and classifies; the scheduler decides what runs next. | `.../outbound/delivery/relay.ex`, `.../delivery/dispatcher.ex` |
...
| Change how a Lua transform is bounded (CPU budget, call depth, string size, breach reporting) | `lib/ash_integration/outbound/delivery/transform/runtime/lua/budget.ex` — the only place that speaks the VM's own limit primitives |
...
5. **The signing secret never enters the Lua runtime sandbox.** It is decrypted
   ...
   - **Sandbox host APIs are pure computation.** Capabilities handed back into
     ...
     call that **burns CPU**; one that **blocks** executes no VM instructions and
     so defeats the step budget, leaving only the outer `Task`. Purity is what
     keeps the ceilings meaningful.

   - **Exhausting the step budget parks the delivery, even when the script
     catches it.** The VM raises the breach as an ordinary Lua error, so `pcall`
     catches it — a script could otherwise burn its whole budget, swallow the
     error, and return a deliverable descriptor. `runtime/lua/budget.ex` closes
     that by checking the instruction tally on the way out of a *successful*
     evaluation as well as a failed one. Total CPU is bounded either way (the
     budget is per top-level evaluation and a caught breach is never refunded);
     what the check preserves is that running out of budget is always an error,
     never a result. The wall-clock ceiling is the outer `Task` — the VM has no
     timer of its own. See `runtime/lua/budget.ex` and
