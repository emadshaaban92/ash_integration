...
**bounded probe pass** that gives sound automatic recovery. Outbound only.
Pre-1.0; no backward-compatibility constraints.
...
3. **Bounded probe pass** (§7) — a separate, infrequent tick that lets one
   delivery through for a bounded, round-robin set of suspended connections,
   reusing the normal claim/relay path.
...
## 7. The bounded probe pass

Recovery needs *observed* success, which needs traffic — but §2 forbids
unbounded traffic to sick connections. The resolution is a **separate,
infrequent tick** (kept out of the hot scheduler so that query stays a one-line
...
config :ash_integration,
  health: [
    ...
    recompute_interval_ms: 60_000,  # how often suspended sets are recomputed (≫ typical delivery latency, so probe successes are visible before re-evaluation)
    probe_interval_ms:    30_000,   # the separate probe tick
    probe_batch:          3         # M — suspended connections probed per tick (≪ delivery concurrency)
  ]
```
