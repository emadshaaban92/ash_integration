defmodule Mix.Tasks.AshIntegration.Events do
  ...
  def run(_args) do
    ...
    if catalog == %{} do
      ...
      |> Enum.each(&print_event_type/1)
    end
  end

  defp print_event_type({event_type, %{versions: versions, producers: producers}}) do
    ...
    producers
    |> Enum.map(fn {resource, action} -> "#{Info.source_resource(resource)}.#{action}" end)
    |> Enum.uniq()
    |> Enum.sort()
