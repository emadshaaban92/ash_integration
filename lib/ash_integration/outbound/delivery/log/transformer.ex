defmodule AshIntegration.Outbound.Delivery.Log.Transformer do
  @moduledoc false
  ...
  def transform(dsl_state) do
    {:ok,
     ...
     |> add_attribute_if_not_exists(:version, :integer, allow_nil?: true, public?: true)
     |> add_attribute_if_not_exists(:event_key, :string, allow_nil?: true, public?: true)
     ...
     |> add_reference_if_not_exists(:event_delivery, ignore?: true)
  ...
  defp add_event_delivery_relationship_if_not_exists(dsl_state) do
    if Info.relationship(dsl_state, :event_delivery) do
      dsl_state
    else
      {:ok, relationship} =
        Transformer.build_entity(Dsl, [:relationships], :belongs_to,
