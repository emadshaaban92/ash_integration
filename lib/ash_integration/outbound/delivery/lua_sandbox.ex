defmodule AshIntegration.Outbound.Delivery.LuaSandbox do
  ...
  script sends those defaults and the script only has to express overrides. If
  the script sets `result` to nil, the event is skipped.
  """
  ...
  def execute(script, event_data, opts) do
    ...
    task =
      Task.async(fn ->
        try do
          run_script(script, event_data, preseed)
        rescue
  ...
  defp preseed_result(lua, nil), do: lua

  defp preseed_result(lua, preseed) do
    {encoded, lua} = Lua.encode!(lua, stringify_keys(preseed))
    Lua.set!(lua, [:result], encoded)
  end
