if Code.ensure_loaded?(Google.Protobuf.FileDescriptorSet) do
  defmodule AshIntegration.Transports.Grpc.ProtoValidator do
    ...
    This module is only compiled when the `:protobuf` dependency is available.
    """
    ...
    defp validate_map_field(field, value, descriptor, file_desc_set, path) do
      ...
      if is_map(value) do
        ...
        Enum.reduce(value, {[], []}, fn {k, v}, {errs, warns} ->
          ...
          {ve, vw} =
            validate_single_value(value_field, v, file_desc_set, entry_path ++ ["value"])
    ...
    defp default_value_text(%FieldDescriptorProto{type: type}) when type in @integer_types,
      do: "0"

    defp default_value_text(%FieldDescriptorProto{type: type}) when type in @float_types,
      do: "0.0"
